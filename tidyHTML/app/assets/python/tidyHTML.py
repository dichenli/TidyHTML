...
def main():
    """pop up a file selection window, ask user to select an html file to convert,
    then convert it, output a .bak file with the same file name"""
...
def tidy_str(html_string):
    """Without prompting input and output, take an input string (as stream from html file)
    and output it in the tidy version"""
    words_list = separate_out_strings(html_string)
    words_list = process_words_list(words_list)
...
def lower_case_tag(tag_string):
    ...
    new_tag = ''
    i = 0
    while i <len(tag_string) and (not tag_string[i].isalpha()): #before start of tagname
        new_tag = new_tag + tag_string[i]
        i += 1
    while i <len(tag_string) and tag_string[i] not in [' ', '\t', '>', '\n', '/']: #stops when tag name ends
