...
def separate_out_strings(str):
    ...
    while(i < str_len):
        ...
        elif str[i] == '<' and i + 2 < len(str) and str[i + 1].isalpha(): #start tags
            j2 = i
        elif str[i] == '<' and i + 3 < len(str) and str[i + 1] == '/' and str[i + 2].isalpha(): #end tags
            j2 = i
        elif(str[i] == '>' and i + 1 < len(str) and str[i + 1] not in ['\t', ' ', '\n']): #end of a tag
            j2 = i + 1
##        elif(str[i] == '\n' and str[j1] != '<'): #to deal with cases like: <a \n>
        elif(str[i] == '\n'):
            j2 = i + 1
        if(j1 < j2):
...
def strip_whole_list(input_list):
    ...
    while i < len(input_list):
        ...
        if input_list[i] == '\n':
            if input_list[i - 1][-1] == '\n' or i == 0: #the if judgements here try to find out an empty line
...
def cut_line_left(words_list, cut_index, cut_point, frozen_strings):
    ...
    j = cut_point
    while i >= 0:
        if not is_tag(words_list[i]) and i not in frozen_strings: #we don't cut tags, or strings protected by <pre>
            while j >= 0:
                if words_list[i][j] in ['\t',' ']:
...
def cut_line_right(words_list, cut_index, cut_point, frozen_strings):                
    ...
    j = cut_point
    while i < len(words_list):
        if not is_tag(words_list[i]) and i not in frozen_strings:
            while j < len(words_list[i]):
                if words_list[i][j] in ['\t',' ']:
                    return make_cut(words_list, i, j) #cut successful, so return the point of new line
                elif words_list[i][j] == '\n':
...
def is_tag(tag_string):
    ...
    #consider case: '<a> blabla <\a>', it will not be judged as a tag given the condition above
        ...
        if string[1].isalpha() or (string[1] == '/' and string[2].isalpha()):
